...
────────────────────────────
Composite real-time score for an OPEN position. Combines:
  • Candle patterns post-entry (25%)
  • VIX velocity (20%)
  • Premium velocity (20%)
  • Day-high/low proximity (15%)
...
def score_candle_patterns(candles: List[Dict], action: str,
    ...
    penalty = round(2.5 * confidence, 2)
    return {
        "penalty": penalty,
...
def score_vix(action: str) -> Dict:
    """Max penalty: -2.0 (20% weight)."""
    a = vix_assess(action)
    raw = a.get("score_penalty", 0)  # 0-3
    penalty = min(2.0, raw * 2.0 / 3.0)
    return {
...
def compute_health(
    *,
    ...
    # Reasons
    reasons = []
    for c in (c_candle, c_vix, c_prem, c_prox, c_time, c_pat):
        if c.get("reason"):
