...
function TrapFinderTab() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      {/* ── TRAP VERDICT — Probability-Based Decision ── */}
      {verdict && ["nifty", "banknifty"].map(key => {
        const v = verdict?.[key];
        if (!v) return null;
        const label = key === "nifty" ? "NIFTY" : "BANKNIFTY";
        const ac = v.action?.includes("CE") ? GREEN : v.action?.includes("PE") ? RED : "#555";
        const cc = v.confidence === "HIGH" ? GREEN : v.confidence === "MEDIUM" ? YELLOW : "#555";
        const t = v.trade || {};
        const bp = v.bullPct || 50;
        const brp = v.bearPct || 50;

        return (
          <Card key={key} style={{ background: "#0D0D15", border: `1px solid ${ac}44` }}>
            {/* Probability Meter */}
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
              <span style={{ color: "#fff", fontWeight: 900, fontSize: 16 }}>{label}</span>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                {v.openType && v.openType !== "UNKNOWN" && <span style={{ color: v.openType === "GAP UP" ? GREEN : v.openType === "GAP DOWN" ? RED : "#888", fontSize: 10, fontWeight: 700 }}>{v.openType}</span>}
                <span style={{ color: "#444", fontSize: 10 }}>{v.timestamp}</span>
              </div>
            </div>

            {/* Bull vs Bear Probability Bar */}
            <div style={{ background: "#0A0A12", borderRadius: 10, padding: "12px 14px", marginBottom: 12 }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
                <div>
                  <span style={{ color: GREEN, fontWeight: 900, fontSize: 22 }}>{bp}%</span>
                  <span style={{ color: "#555", fontSize: 10, marginLeft: 6 }}>BULLISH</span>
                </div>
                <div style={{ textAlign: "center" }}>
                  <span style={{ background: ac + "22", color: ac, padding: "4px 16px", borderRadius: 6, fontSize: 14, fontWeight: 900 }}>{v.action}</span>
                  <div style={{ color: cc, fontSize: 10, fontWeight: 700, marginTop: 2 }}>{v.confidence} ({v.winProbability}% edge)</div>
                </div>
                <div style={{ textAlign: "right" }}>
                  <span style={{ color: RED, fontWeight: 900, fontSize: 22 }}>{brp}%</span>
                  <span style={{ color: "#555", fontSize: 10, marginLeft: 6 }}>BEARISH</span>
                </div>
              </div>
              <div style={{ display: "flex", height: 10, borderRadius: 5, overflow: "hidden", background: "#1a1a25" }}>
                <div style={{ width: `${bp}%`, background: `linear-gradient(90deg, ${GREEN}, ${GREEN}88)`, transition: "width 0.5s" }} />
                <div style={{ width: `${brp}%`, background: `linear-gradient(90deg, ${RED}88, ${RED})`, transition: "width 0.5s" }} />
              </div>
              {v.action === "NO TRADE" && <div style={{ color: "#666", fontSize: 11, textAlign: "center", marginTop: 6 }}>Need {">"}60% edge for trade — too close to call</div>}
            </div>

            {/* Trade Card — Only if actionable */}
            ...
            {v.reasons?.length > 0 && (
              <div style={{ marginBottom: 10 }}>
                <div style={{ color: ac, fontSize: 11, fontWeight: 900, marginBottom: 6 }}>WHY {v.action}</div>
                {v.reasons.map((r, i) => (
                  <div key={i} style={{ color: "#ccc", fontSize: 11, marginBottom: 4, paddingLeft: 8, borderLeft: `2px solid ${ac}33` }}>{r}</div>
                ))}
              </div>
            )}

            {/* AGAINST — What opposes */}
            {v.against?.length > 0 && (
              <div style={{ marginBottom: 10 }}>
                <div style={{ color: "#666", fontSize: 10, fontWeight: 700, marginBottom: 4 }}>AGAINST</div>
                {v.against.map((r, i) => (
                  <div key={i} style={{ color: "#666", fontSize: 10, marginBottom: 2, paddingLeft: 8 }}>{r}</div>
                ))}
              </div>
            )}
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
              {v.currentExpiry?.length > 0 && (
                <div style={{ flex: 1, background: "#111118", borderRadius: 8, padding: "8px 10px" }}>
                  <div style={{ color: ACCENT, fontSize: 9, fontWeight: 700, marginBottom: 4 }}>THIS EXPIRY</div>
                  {v.currentExpiry.map((p, i) => <div key={i} style={{ color: "#888", fontSize: 10, marginBottom: 2 }}>{p}</div>)}
                </div>
              )}
              {v.nextWeek?.length > 0 && (
                <div style={{ flex: 1, background: "#111118", borderRadius: 8, padding: "8px 10px" }}>
                  <div style={{ color: ORANGE, fontSize: 9, fontWeight: 700, marginBottom: 4 }}>NEXT WEEK</div>
                  {v.nextWeek.map((p, i) => <div key={i} style={{ color: "#888", fontSize: 10, marginBottom: 2 }}>{p}</div>)}
                </div>
              )}
            </div>
